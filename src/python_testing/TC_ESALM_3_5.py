...
ALARM_NAME = "OverPower"
ALARM_BIT = int(AlarmBitmap.kOverPower)


class TC_ESALM_3_5(ElectricalAlarmTestBaseHelper):

    def pics_TC_ESALM_3_5(self) -> list[str]:
        return ["ESALM.S", "ESALM.S.F25", "ESALM.S.E0000"]

    def steps_TC_ESALM_3_5(self) -> list[TestStep]:
        return self.alarm_lifecycle_steps(ALARM_NAME)

    @run_if_endpoint_matches(has_feature(cluster, cluster.Bitmaps.Feature.kOverPower))
    async def test_TC_ESALM_3_5(self):
        """[TC-ESALM-3.5] OverPower alarm lifecycle with Server as DUT

        Verifies the lifecycle of the OverPower alarm: the condition sets its bit in State and
